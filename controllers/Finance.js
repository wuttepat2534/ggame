    ...
    static CheckInformation(restest, dataUser) {
        return new Promise((resolve, reject) => {
                            ...
                            if (data.length !== 0 || data.length > 0) {
                                ...
                                connection.query(sql_LogDeposit, async (error, logDeposit_transRef) => {
                                    if (error) {
                                        ...
                                        if (dataLog.length < 1) {
                                            let sql_NameAccount = `SELECT * FROM member WHERE SUBSTRING(accountNumber, 6, 4) = '${last4Digits}' AND bank = '${Bank}' AND phonenumber = '${dataUsers.phonenumber}'`;
            ...
            if (Bank === 'ธนาคารกรุงเทพ') {
                const data = resFinance.data.receiver.account.value;
                const last4DigitBG = data.replace(/\D/g, '').slice(-3);
                const dataM = resFinance.data.sender.account.value
                const last4DigitMBG = dataM.replace(/\D/g, '').slice(-3);
                //console.log(last4DigitBG);
                let sql_deposit = `SELECT * FROM depositaccount WHERE activestatus = "เปิดใช้งาน" AND RIGHT(accountNumber, 3) = '${last4DigitBG}'`;
                connection.query(sql_deposit, (error, depositData) => {
                    try {
                        if (error) {
                            console.log(error);
                            ...
                            if (data.length !== 0 || data.length > 0) {
                                ...
                                connection.query(sql_LogDeposit, async (error, logDeposit_transRef) => {
                                    if (error) {
                                        ...
                                        const dataLog = logDeposit_transRef;
                                        if (dataLog.length < 1) {
                                            //console.log(last4DigitMBG, dataUsers.phonenumber)
                                            let sql_NameAccount = `SELECT * FROM member WHERE RIGHT(accountNumber, 3) = '${last4DigitMBG}' AND bank = '${Bank}' AND phonenumber = '${dataUsers.phonenumber}'`;
                                            connection.query(sql_NameAccount, async (error, nameAccount) => {
                                                if (error) {
                                                    console.log(error);
                                                    //reject(error);
